...
class point_checkpoint : ScriptBaseAnimating
{
	...
	void Precache()
	{
		...
		g_Game.PrecacheModel( "sprites/singularity/prtl/kz_exit1.spr" );
		
		g_SoundSystem.PrecacheSound( "../sound/oghs/misc/cp02_03.mp3" );
		g_SoundSystem.PrecacheSound( "singularity/misc/teleport.wav" );
		g_SoundSystem.PrecacheSound( "ambience/port_suckout1.wav" );
		
		if( string( self.pev.message ).IsEmpty() )
			self.pev.message = "singularity/misc/teleport.wav";
	...
	void RespawnStartThink()
	{
		...
		@m_pSprite = g_EntityFuncs.CreateSprite( "sprites/singularity/prtl/kz_exit1.spr", self.pev.origin, true, 10 );
		m_pSprite.TurnOn();
		m_pSprite.pev.rendermode = kRenderTransAdd;
		m_pSprite.pev.renderamt = 128;
	
		g_SoundSystem.EmitSound( self.edict(), CHAN_STATIC, "singularity/misc/teleport.wav", 1.0f, ATTN_NORM );
