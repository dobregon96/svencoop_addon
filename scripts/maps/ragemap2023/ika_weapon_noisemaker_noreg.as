...
namespace IkaNoiseNoreg
{

const int HLW_SLOT					= 5;
const int HLW_POSITION				= 11;
const int DAMAGE_HANDGRENADE		= 1;
const int HANDGRENADE_DEFAULT_GIVE	= 3;
...
class weapon_noisenoreg : ScriptBasePlayerWeaponEntity
{
    ...
    void Precache()
    {
        ...
        g_Game.PrecacheOther( "noisenoreg" );
    }
    ...
    bool AddToPlayer( CBasePlayer@ pPlayer )
    {
        ...
        NetworkMessage m( MSG_ONE, NetworkMessages::WeapPickup, pPlayer.edict() );
            m.WriteLong( g_ItemRegistry.GetIdForName("weapon_noisenoreg") );
        m.End();
    ...
    void CustomTouch( CBaseEntity@ pOther )
    {
        ...
        if( pPlayer.HasNamedPlayerItem("weapon_noisenoreg") !is null )
        {
              if( pPlayer.GiveAmmo(3, "weapon_noisenoreg", HANDGRENADE_MAX_CARRY) != -1 )
            {
                    self.CheckRespawn();
...
class noisenoreg : ScriptBaseMonsterEntity
{
...
noisenoreg ShootTimed( entvars_t@ pevOwner, Vector vecStart, Vector vecVelocity, float time )
{
    CBaseEntity@ cbeGrenade = g_EntityFuncs.CreateEntity( "noisenoreg", null,  false );
    noisenoreg@ pGrenade = cast<noisenoreg@>(CastToScriptClass(cbeGrenade));
...
void Register()
{
    g_CustomEntityFuncs.RegisterCustomEntity( "IkaNoiseNoreg::noisenoreg", "noisenoreg" );
    g_CustomEntityFuncs.RegisterCustomEntity( "IkaNoiseNoreg::weapon_noisenoreg", "weapon_noisenoreg" );
    g_ItemRegistry.RegisterWeapon( "weapon_noisenoreg", "ragemap2023/ika_weapons", "weapon_noisenoreg", "", "weapon_noisenoreg" );
}
