...
class CMonsterkateCustom : ScriptBaseMonsterEntity
{
	...
	void Spawn()
	{
		...
		self.SetPlayerAlly( !self.IsPlayerAlly() );
		self.CanPlayerFollow();
		
		if( self.IsPlayerAlly() ){ SetUse( UseFunction( this.FollowerUse ) ); }
		self.MonsterInit();
	}
	...
	void FollowerUse( CBaseEntity@ pActivator, CBaseEntity@ pCaller, USE_TYPE useType, float flValue )
	{
		...
		if( pTarget is pActivator )
		{
			...
			//g_EngineFuncs.ServerPrint("-- DEBUG: CanPlayerFollow state is: " + self.CanPlayerFollow() + " and IsPlayerAlly status is: " + self.IsPlayerAlly() + " \n");
	}
}
...
{
	InitSchedules();
	g_CustomEntityFuncs.RegisterCustomEntity( "kateCustom::CMonsterkateCustom", "monster_kate" );
}
