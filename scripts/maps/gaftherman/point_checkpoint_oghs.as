...
class point_checkpoint : ScriptBaseAnimating  
{
    ...
    string ActivationMusic = 'singularity/misc/cp_life.mp3';
    string PlayerSpawnSound = 'singularity/misc/teleport.wav';
    string SpawnSprite = 'sprites/singularity/prtl/kz_exit1.spr';
    ...
    void Spawn()
    {
        ...
        g_EntityFuncs.SetModel(self, string( self.pev.model ).IsEmpty() ? string_t( self.pev.model = 'models/singularity/icons/mk_logo_purple.mdl') : self.pev.model);
        g_EntityFuncs.SetSize(self.pev, (self.pev.vuser1 != g_vecZero) ? self.pev.vuser1 : Vector(-4,-4,-8), (self.pev.vuser1 != g_vecZero) ? self.pev.vuser2 : Vector(4,-4,-8));
    ...
    void Precache()
    {
        g_Game.PrecacheModel(string(self.pev.model).IsEmpty() ? string_t(self.pev.model = 'models/singularity/icons/mk_logo_purple.mdl') : self.pev.model);
        g_Game.PrecacheGeneric(string(self.pev.model).IsEmpty() ? string_t(self.pev.model = 'models/singularity/icons/mk_logo_purple.mdl') : self.pev.model);

        g_Game.PrecacheModel(SpawnSprite);
        g_Game.PrecacheGeneric(SpawnSprite);

        g_SoundSystem.PrecacheSound(string(self.pev.message).IsEmpty() ? string_t(self.pev.message = 'singularity/misc/teleport.wav') : self.pev.message);
        g_Game.PrecacheGeneric(string(self.pev.message).IsEmpty() ?  string_t(self.pev.message = 'sound/singularity/misc/teleport.wav') : string_t('sound/' + self.pev.message));
