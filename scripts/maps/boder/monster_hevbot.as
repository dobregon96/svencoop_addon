const int hevbot_heal_other = 1;
const int hevbot_heal_self = 2;
const int hevbot_die = 3;


class monster_hevbot : ScriptBaseMonsterEntity
{
	...
	{
		Precache( ); 
		g_EntityFuncs.SetModel( self, "models/ns/hevbot.mdl" );
		...
		g_EntityFuncs.DispatchKeyValue( self.edict(), "displayname", "HEVbot" );
	...
	void Precache()
	{
		g_Game.PrecacheModel( "models/ns/hevbot.mdl" );
	...
	void RemoveHealthKit ( CBaseEntity@ pHealthkit )
	{
		...
		switch ( pEvent.event ) 
		{
			...
			{	
				if(ammo >= 1)
				{
					...
					g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_VOICE, "items/suitchargeok1.wav", 0.8, ATTN_NORM, 0, 95 + Math.RandomLong( 0, 10 ) );
					
					te_beamentpointg(pTarget, self.pev.origin);
					
					pTarget.pev.armorvalue += 5;
					if(pTarget.pev.armorvalue >= 100){ pTarget.pev.armorvalue = 100; }
...
string GetHevbotName()
{
	return "monster_hevbot";
}

void RegisterHevbot()
{
	g_CustomEntityFuncs.RegisterCustomEntity( "monster_hevbot", GetHevbotName() );
}
