...
// Models
const string g_PeeMdl				= "models/scmod/weapons/ut99/ut2k4shock/p_gauss.mdl";
const string g_VeeMdl				= "models/scmod/weapons/ut99/ut2k4shock/v_shock.mdl";
const string g_WeeMdl				= "models/scmod/weapons/ut99/ut2k4shock/w_gauss.mdl";
...
const string g_SpinSnd				= "ambience/rotormachine.wav";
const string g_ShootSnd				= "scmod/weapons/ut99/shock.ogg";
const string g_EmptySnd				= "weapons/357_cock1.wav"; //NO!

// Sprites
const string g_GlowSpr				= "sprites/scmod/ut99/shockwall.spr";
const string g_BallSpr				= "sprites/hugeball.spr";
const string g_BeamSpr				= "sprites/scmod/ut99/shocktrail.spr";

// Weapon info
const uint g_MaxCarry				= 50;
...
const float g_Damage				= g_EngineFuncs.CVarGetFloat( "sk_plr_secondarygauss" ); // 190 default in skill.cfg
...
class weapon_clgauss : ScriptBasePlayerWeaponEntity
{
	...
	void PrimaryAttack()
	{
		...
		self.m_flNextPrimaryAttack = self.m_flNextSecondaryAttack = g_Engine.time + 0.8f; // m_pPlayer.m_flNextAttack = g_Engine.time + 0.2f;
	}
	...
	void StartFire()
	{
		...
		{
			flDamage = g_Damage * 2; //200 NOTE: NO NO, this is an extreme damage amount!! will just go from 10 to 2 now
		}
		else
		{
			flDamage = ( g_Damage * 2 ) * ( ( g_Engine.time - m_flStartCharge ) / GetFullChargeTime() ); //200
		}

		if( m_bPrimaryFire )
		{
			// fixed damage on primary attack
			//flDamage = 20;  nope i'm using secondary damage always so its 190
			flDamage = g_Damage;
		}
		...
		//{
			...
			if( !m_bPrimaryFire )
			{
				m_pPlayer.pev.velocity = m_pPlayer.pev.velocity - g_Engine.v_forward * flDamage * 2;  //changed 5 to 2
			}
	...
	void Fire( Vector vecOrigSrc, Vector vecDir, float flDamage )
	{
		...
		g_SoundSystem.PlaySound( m_pPlayer.edict(), CHAN_WEAPON, g_ShootSnd, 0.5f + flDamage * ( 0.10 / 400.0f ), ATTN_NORM, 0, 85 - Math.RandomLong( 0, 0x1f ) );  //changed 0.40 to 0.10
		...
		while( flDamage > 10 && nMaxHits > 0 )
		{
			...
			if( fFirstBeam )
			{
				...
				NetworkMessage beampoint( MSG_BROADCAST, NetworkMessages::SVC_TEMPENTITY, tr.vecEndPos );
					...
					beampoint.WriteByte( 2 ); // How long to display the beam (0.1)  *10
					beampoint.WriteByte( m_bPrimaryFire ? 55 : 25 ); // Width of the beam (1.0, 2.5) * 100
					beampoint.WriteByte( 0 ); // Noise amplitude. (SC's is 0.1) * 100
					beampoint.WriteByte( m_bPrimaryFire ? 100 : 255 ); // Red color
					beampoint.WriteByte( m_bPrimaryFire ? 55 : 255 ); // Green color
					beampoint.WriteByte( m_bPrimaryFire ? 200 : 255 ); // Blue color
					beampoint.WriteByte( m_bPrimaryFire ? 255 : int( flDamage ) ); // Brightness
			...
			else // Beam reflection -R4to0
			{
				...
				NetworkMessage beampoints( MSG_BROADCAST, NetworkMessages::SVC_TEMPENTITY, null );
					...
					beampoints.WriteByte( 2 ); // How long to display the beam (0.1) *10
					beampoints.WriteByte( m_bPrimaryFire ? 55 : 25 ); // Width of the beam (1.0, 2.5) * 100
					beampoints.WriteByte( 0 ); // Noise amplitude. (SC's is 0.1) * 10
					beampoints.WriteByte( m_bPrimaryFire ? 100 : 255 ); // Red color
					beampoints.WriteByte( m_bPrimaryFire ? 55 : 255 ); // Green color
					beampoints.WriteByte( m_bPrimaryFire ? 200 : 255 ); // Blue color
					beampoints.WriteByte( m_bPrimaryFire ? 255 : int( flDamage ) ); // Brightness
	...
	{
		NetworkMessage glow( MSG_BROADCAST, NetworkMessages::SVC_TEMPENTITY );
			glow.WriteByte( TE_EXPLOSION );
			...
			glow.WriteByte( 8 ); //scale
			glow.WriteByte( 20 ); //framerate
			glow.WriteByte( TE_EXPLFLAG_NOSOUND );
		glow.End();
	}
	...
	void GaussBall( TraceResult &in tr, Vector vecEnd, uint iCount, float flLife, uint iAmplitude, uint iSpeed )
	{
		NetworkMessage gaussball( MSG_BROADCAST, NetworkMessages::SVC_TEMPENTITY );
			...
			gaussball.WriteByte( int( flLife * 2 ) ); // Time to wait before fading out 
...
void Register()
{
	...
	g_ItemRegistry.RegisterWeapon( g_WeaponName, "scmod/ut99", g_PriAmmoType );
}
