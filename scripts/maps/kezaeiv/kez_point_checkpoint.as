...
enum PointCheckpointFlags2
{
	SF_CHECKPOINT_REUSABLE 		= 1 << 0,	//This checkpoint is reusable
}

class point_checkpoint2 : ScriptBaseAnimating
{
	...
	void Precache()
	{
		...
		if( string( self.pev.model ).IsEmpty() )
			g_Game.PrecacheModel( "models/kezaeiv/icons/mk_logo_crimson.mdl" );
		else
			g_Game.PrecacheModel( self.pev.model );
		
		g_Game.PrecacheModel( "sprites/kezaeiv/prtl/kz_exit1.spr" );
		
		g_SoundSystem.PrecacheSound( "../sound/kezaeiv/misc/cp_nod_reinf.wav" );
	...
	void Spawn()
	{
		...
		// Allow for custom models
		if( string( self.pev.model ).IsEmpty() )
			g_EntityFuncs.SetModel( self, "models/kezaeiv/icons/mk_logo_crimson.mdl" );
		else
	...
	void Touch( CBaseEntity@ pOther )
	{
		...
		g_SoundSystem.EmitSound( self.edict(), CHAN_STATIC, "../sound/kezaeiv/misc/cp_nod_reinf.wav", 1.0f, ATTN_NONE );
...
void RegisterPointCheckPointEntity2()
{
	g_CustomEntityFuncs.RegisterCustomEntity( "point_checkpoint2", "point_checkpoint2" );
	
	g_Game.PrecacheOther( "point_checkpoint2" );	
}